...
/**
 * Pinpoint forward/backward drift test.
 *
 * Auto-oscillates forward and backward 24 inches (robot frame).
 * Uses setTeleOpDrive with fieldCentric=true so robot-forward input
 * always moves the robot in its forward direction.
 * Tracks cross-axis drift which should be ~0 during pure forward/backward.
 *
 * Controls:
 ...
 */
@Config
@TeleOp(name = "PinpointForwardTest", group = "V2")
public class PinpointForwardTest extends OpMode {
    ...
    private static final String TAG = "ForwardTest";
    ...
    public void init() {
        ...
        telemetry.addLine("PinpointForwardTest ready");
    ...
    public void loop() {
        ...
        // Distance from last turnpoint along the travel axis
        ...
        if (running) {
            if (lapCount >= MAX_LAPS) {
                ...
                // Reverse direction, record new turnpoint
            ...
            // Slow down near turnaround
            double distToEnd = OSCILLATE_DISTANCE - distTraveled;
            double speedScale = Math.min(1.0, distToEnd / SLOW_ZONE);
            double speed = DRIVE_SPEED * (0.15 + 0.85 * speedScale);

            // Robot-frame forward/backward via fieldCentric=true
            double fwd = goingForward ? speed : -speed;
            follower.setTeleOpDrive(fwd, 0, 0, true);
        } else {
        ...
        // At heading 90°, forward = +Y, so cross-axis = X drift
        double driftCross = Math.abs(x - START_X);
        double distMain = y - START_Y;
        ...
        telemetry.addData("Status", running ? (goingForward ? "FORWARD" : "BACKWARD") : "STOPPED");
